...
function selectCategory(name) {
  ...
  updateSamplesGrid(latestCategories);
  drawer.clear();
}
...
document.getElementById('btn-add-sample').addEventListener('click', async () => {
  ...
  socket.emit('add-sample', { category: currentCategory, features, thumbnail });

  drawer.clear();
  updateSamplesGrid();
});

function updateSamplesGrid(serverCategories) {
  const grid = document.getElementById('samples-grid');
  const catData = serverCategories ? serverCategories[currentCategory] : null;
  const thumbnails = catData ? (catData.thumbnails || []) : [];
...
socket.on('custom-categories', (categories) => {
  ...
  // Refresh grid if viewing a category
  if (currentCategory) {
    updateSamplesGrid(categories);
  }
});
